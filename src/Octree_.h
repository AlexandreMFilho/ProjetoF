...
extern int total_folhas_pos_merge;
extern int folhas_vazias_pos_merge;
extern int folhas_iso_pos_merge;
extern int total_nos_pos_merge;
extern bool debug;
...
public:
	...
	void metricas_octree_pos_merge(Octree*);
	void determinaTopologia(Octree*);
	void carregaVariaveisGlobais(Octree*);
...
Octree::Octree(int x1, int y1, int z1, int x2, int y2, int z2,Octree* pai,float iso, int lvl_atual,int lvl_max,int identidade)
{
	...
	//Chegou no nível máximo, carrega o nó folha.
		...
		if((cont_pos_vert_ != 8)&&(cont_neg_vert_ != 8)){
			...
			this->temIsoDentro = true;
			//Neste ponto isso somente ocorre nos nos folha da arvore octree
		}
		//Eu poderia fazer isso, mas ja eh feito em sinaliza_octree()
//		if((cont_pos_vert_ == 8)&&(cont_neg_vert_ == 8)){
//			this->sinal = 0;
//			this->temIsoDentro = false;
//			this->topology_ = EMPTY;
//		}
	}
}

void Octree::metricas_octree_pos_merge(Octree* oct){
    if(oct == nullptr) return;

    // Conta todos os nós (pais e folhas)
    total_nos_pos_merge++;

    if(oct->isfather){
        for(int i=0; i<8; i++){
            metricas_octree_pos_merge(oct->children[i]);
        }
    } else {
        // Chegou em um nó folha
        total_folhas_pos_merge++;

        if(oct->temIsoDentro){
            folhas_iso_pos_merge++;
        } else {
            folhas_vazias_pos_merge++;
        }
    }
}
...
int Octree::sinaliza_octree(Octree* oct){
	...
	if(oct->nivel == oct->nivel_max){
		...
		if((oct->cont_pos_vert_==8)||(oct->cont_neg_vert_==8)){
			oct->temIsoDentro = false; //No com os 8 vertices positivos ou negativos
			cubos_vazios_total++;
			oct->topology_ = EMPTY;
			oct->sinal = 0;
			return oct->sinal;
			...
			carregaVariaveisGlobais(oct);
			oct->topology_ = topology(oct);
			oct->sinal = 1;
			oct->temIsoDentro = true;
			return oct->sinal;
		}
	}
	...
	//nos filhos, necessário ir neles.
		...
		if(cont == 0){
			//Filhos vazios, posso fazer merge dos meus 8 filhos e olhar agoa so para o pai
			oct->sinal = 0;
			oct->temIsoDentro = false;
...
void executeMerge(Octree * oct){
    oct->isfather = false;

    if(oct->children[0] != nullptr) oct->merge_step = oct->children[0]->merge_step * 2;

    // O MAPEAMENTO MÁGICO: Traduz a ID do filho para a quina correta do 'cube'
    oct->cube[0] = oct->children[0]->cube[0];
    oct->cube[1] = oct->children[3]->cube[1]; // Filho 3 fica em (0,0,1)
    oct->cube[2] = oct->children[7]->cube[2]; // Filho 7 fica em (0,1,1)
    oct->cube[3] = oct->children[4]->cube[3]; // Filho 4 fica em (0,1,0)
    oct->cube[4] = oct->children[1]->cube[4]; // Filho 1 fica em (1,0,0)
    oct->cube[5] = oct->children[2]->cube[5]; // Filho 2 fica em (1,0,1)
    oct->cube[6] = oct->children[6]->cube[6]; // Filho 6 fica em (1,1,1)
    oct->cube[7] = oct->children[5]->cube[7]; // Filho 5 fica em (1,1,0)

    // Zera para recalcular
    oct->cont_pos_vert_ = 0;
    oct->cont_neg_vert_ = 0;
    oct->cont_zero_vert_ = 0;

    for(int i=0; i<8; i++){
        // Conta os vértices usando a matriz já desembaralhada!
        if(oct->cube[i] > 0.0) oct->cont_pos_vert_++;
        else if(oct->cube[i] < 0.0) oct->cont_neg_vert_++;
        else oct->cont_zero_vert_++;

        delete oct->children[i];
        oct->children[i] = nullptr;
    }
}

void Octree::merge(Octree * oct){
	static int cont_merge = 0;
	bool todosSimples = true, domerge = true,podemerge = true,vazio = true;

	cont_merge++;

	if(cont_merge % 100000 == 0){
		if(debug)printf("Ja visitou %d nos...\n", cont_merge);
	}

	if((oct == nullptr)or(!oct->isfather)){
		return;
	}

	for(int i = 0; i< 8; i++){
		if(oct->children[i] != nullptr){
			merge(oct->children[i]);
		}
	}


	//Percorre os filhos pegando a topologia e checando se todos estao vazios ou se sao todos simples
	for(int i =0;i<8;i++){
		if(oct->children[i] == nullptr){
		    podemerge = false;
		    break;
		}
		if(oct->children[i]->isfather){
			podemerge = false;
		}
		if(oct->children[i]->temIsoDentro == true){
			vazio = false;
		}
	...
	if(podemerge){
		...
		else if(todosSimples && oct->nivel >= oct->nivel_max -1){
			oct->temIsoDentro = true;
			executeMerge(oct);
			carregaVariaveisGlobais(oct);
			oct->topology_ = topology(oct);
 ...
 *fazer o merge com os filhos.
	Se todos vazios, entao fazer o merge.
	 merge {
	 Incializar os vertices do no pai com vertices I dos filhos I.
	Matar os 8 filhos.
	}
	Se tiver algum dado la dentro.
		Se o dado for LEAF ou seja, topologia simples
			fazer o merge
		senao
			mantem topology = -1 (tambem significa que o no nao eh folha da octree
*/
...
void Octree::determinaTopologia(Octree* oct){
	std::vector<AddPoints> extrapoints;
	extrapoints.clear();
...
void Octree::visualiza_octree_F(Octree* oct,FILE* arq)
{
	...
	fprintf(arq,"(Nivel:%d , V_0:%d%d%d ,V_6:%d%d%d, sig:%d, topology:%d, isfather:%d, mergeStep:%d, nivelMaximo:%d)\n",oct->nivel,oct->V_0->x,oct->V_0->y,oct->V_0->z,oct->V_6->x,oct->V_6->y,oct->V_6->z,oct->sinal,oct->topology_,oct->isfather,oct->merge_step,oct->nivel_max);
...
float Octree::get_data(const int i, const int j, const int k)
{
    ...
    return f_grid_data[i + j * size_x + k * size_x * size_y]; // Corrigido!
}
...
void Octree::mesh(Octree* oct){
	...
	if(oct->isfather){
		for(int i=0;i<8;i++){
			if(oct->children[i] !=nullptr and oct->children[i]->temIsoDentro){
				mesh(oct->children[i]);
			}
		}
	}else{
		if(oct->temIsoDentro){
			determinaTopologia(oct);
		}
	}
}

void Octree::carregaVariaveisGlobais(Octree* oct){
    for(int i = 0; i < 8; i++){
        _cube[i] = oct->cube[i];
    }
    cont_pos_vert = oct->cont_pos_vert_;
