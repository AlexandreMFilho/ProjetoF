...
void executeMerge(Octree * oct){
	...
	for(int i=0;i<8;i++){
		oct->cube[i] = oct->children[i]->cube[i]; //atualizacao de coordenadas ou isovalor dos pontos do cubo
		p = oct->children[i];
		p->parent = nullptr;
		oct->children[i] = nullptr;
		//delete(p);
		//Aqui eu coloco toda mudanca que pegue algo dentro do cubo que vai morrer
	}
...
void Octree::merge(Octree * oct){
	...
	if((oct->nivel == oct->nivel_max - 1)){
		...
		//SENAO Verificar se são casos simples
			...
			//Filhos sao da topologia folha, pode fazer merge e esse no agora tambem eh folha da topologia LEAF
			if(domerge){
				oct->topology_ = LEAF;
//				executeMerge(oct);
			}
